...
async function callMistral(prompt: string) {
  const mistralRes = await fetch('https://api.mistral.ai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'mistral-tiny',
      messages: [
        { role: 'system', content: 'You are a helpful assistant that follows instructions exactly.' },
...
function removeEmojis(text: string): string {
  ...
  return text.replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{2300}-\u{23FF}\u{2B50}\u{2B06}\u{2934}\u{2935}\u{25A0}-\u{25FF}\u{1F1E6}-\u{1F1FF}]/gu, '');
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const outline = await callMistral(outlinePrompt);
    ...
    let article = await callMistral(articlePrompt);
    article = removeEmojis(article);
